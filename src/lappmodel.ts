...
export class LAppModel extends CubismUserModel {
   ...
   * @param fileName
   */
  public loadAssets(dir: string, model: string, name: string): Promise<void> {
    return new Promise(resolve => {
      let fileName: string = model + '.model3.json';
      this._realName = name;
  ...
  public startMotion(
    ...
    //TODO:用名字当name，不是idle_0
    const no = this._modelSetting.getMotionFileIndex(group, emotion);
    LAppPal.printMessage("[APP]start motion." + `${group}_${no}`);


    if (no == -1){
      //读取通用motion
      let commonModel=LAppCommonModel.getInstance();
      let motion: CubismMotion = commonModel._common_motions.getValue(emotion) as CubismMotion;
      if(motion!=null){
   ...
   */
  public doDraw(bright: number): void {
    ...
    this.getRenderer().drawModel(this.getOpacity(), bright);
  }

  /**
   * モデルを描画する処理。モデルを描画する空間のView-Projection行列を渡す。
   */
  public draw(matrix: CubismMatrix44, bright: number): void {
    if (this._model == null) {
      return;
    }
  ...
  public constructor() {
    ...
    this._offset = 0;
    this._sliper = true;//active的时候，sliper是true的：只有人物当时显示（被挤走），才会移动。
