...
export namespace Live2DCubismFramework {
  ...
  enum FrequestNode {
    ...
    //FrequestNode_MotionIndex, // getRoot().getValueByString(FileReferences).getValueByString(MotionIndex)
  ...
  export class CubismModelSettingJson extends ICubismModelSetting {
    ...
    public getMotionFileIndex(groupName: string, emotion: string):number{
      if (!this.isExistMotionGroupName(groupName)) {
        return -1;
      }
      let arr = this._jsonValue
      .at(FrequestNode.FrequestNode_Motions)
      .getValueByString(groupName);
      let sum = arr.getVector().getSize();
      for(var i=0;i<sum;i++){
        let item = arr.getValueByIndex(i).getValueByString(FilePath).getRawString();
        if(item.split('motions/')[1].split('.motion3.json')[0]==emotion){
          return i;
        }
      }
      if(i>=arr.getVector().getSize()){
        return -1;
      }
    }
